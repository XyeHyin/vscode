// // name: P_3200_HNOI_2009_有趣的数列.cpp
// // date 2025-06-30
// // author: XyeHyin
...
// }
// vector<int> f(N), g(N);
// int C(int n, int m, int p) { return (f[n] * g[m] * g[n - m] % p); }
...
//     for (int i = 1; i <= p; i++) {
//         f[i] = f[i - 1] * i % p;
//         g[i] = g[i - 1] * fastPow(i, p - 2, p) % p;
//     }
//     int tmp = lucas(2 * n, n, p);
//     cout << tmp * inv(n + 1, p) % p << endl;
//     return;
// }
