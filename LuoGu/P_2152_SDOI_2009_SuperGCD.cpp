// name: P_2152_SDOI_2009_SuperGCD.cpp
// date 2025-06-21
// author: XyeHyin
...
struct BIGINT {
    static const itn base = 10000, base_digits = 4;
    ...
    friend ostream &operator<<(ostream &os, const BIGINT &v) {
        ...
        if (v.a.empty()) {
            ...
            for (itn i = (itn)v.a.size() - 2; i >= 0; --i) {
                snprintf(buf, sizeof(buf), "%04d", v.a[i]);
                os << buf;
            }
        }
        return os;
    ...
    static pair<BIGINT, BIGINT> divmod(const BIGINT &u, const BIGINT &v) {
        ...
        for (itn i = itn(a.a.size()) - 1; i >= 0; --i) {
            ...
            if (r.a.size() >= b.a.size()) {
                ...
                long long r_lo = (r.a.size() > j - 1 ? r.a[j - 1] : 0);
