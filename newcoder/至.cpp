...
#define RESET "\033[0m"
#define dbg(x) std::cout << YELLOW << #x << ":" << x << RESET << " "
#define dbgl(x) std::cout << YELLOW << #x << ":" << x << RESET << "\n"
...
void solve(){
    int n, x1,y1,x2,y2;
    cin>> n >> x1 >> y1 >> x2 >> y2;
    int disa=abs(2-x1)+abs(n-y1);
    int disb=abs(2-x2)+abs(n-y2);
    if(abs(disa-disb)==0){
        YES();
        return;
    }
    if((x1==x2-1 and y1==y2-1 and y1!=n-1 and y2!=n-1)){
        YES();
        return;
    }
    NO();
