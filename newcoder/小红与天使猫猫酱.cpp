...
void solve() {
    int n;
    cin >> n;
    int k = 2 * fastPow(9, mod1 - 2, mod1) % mod1;
    int ans = k * k % mod1 * (100 * ((fastPow(100, n, mod1) - 1 + mod1) % mod1) % mod1 * fastPow(99, mod1 - 2, mod1) % mod1) % mod1;
    ans = (ans+ (2 * k % mod1 * ((1 - k + mod1) % mod1) % mod1) * (10 * ((fastPow(10, n, mod1) - 1 + mod1) % mod1) % mod1 * fastPow(9, mod1 - 2, mod1) % mod1) % mod1)% mod1;
    ans = (ans + (((k - 1 + mod1) % mod1) * ((k - 1 + mod1) % mod1) % mod1 *  (n % mod1) % mod1)) % mod1;
    cout << ans << endl;
    return;
}
