...
long long fastPow(long long base, long long power, long long mo) {
    long long result = 1;
    while (power > 0) {
        if (power & 1) {
...
void solve() {
    ...
    int tmpp = n / 3;
    int modd = n % 3;
    int dic = fastPow(cnt1, tmpp * 2, mod) * fastPow(cnt2, tmpp, mod) % mod;
    tmp = dic;
    if (modd == 1) {
        tmp = (tmp * cnt1) % mod;
    }
    ...
    ans = (ans + fastPow(cnt2, n, mod)) % mod;
